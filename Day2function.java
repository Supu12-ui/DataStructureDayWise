...
public class Day2function {
    ...
    int multiply(int n,int m,int b)
    {
        int ans=0,pow=1;
       while(m!=0)
       {
           int sum=decimalmultiply(n,m%10);
           m=m/10;
           ans=ans+decimaladdition(ans,sum*pow);
           pow=pow*10;
       }
       return  ans;
    }
    int decimalmultiply(int n,int m,int b)
    {
        int ans=0,pow=1,carry=0;
        while(n!=0)
        {
            int sum=carry+((n%10)*m);
            n=n/10;
            sum=sum%10;
            carry=sum/b;
            ans+=sum*pow;
